...
looplire:
	li t0, maxLines
	bge s1, t0, trier	# for(i=0; i<maxLines; i++) {

	slli s2, s1, 3
	add s2, s2, s0
	ld s2, (s2)		# s2=adresse de la ligne courante
...
loopi:
	bge s4,s1, affiche	# for (i=0; i<tablen; i++) { // grande boucle
	li s5,1			# indice loop J
	
loopj:
	bge s5,s1, conti	#   for(j=1; j<tablen; j++) { // petite boucle
	
	slli s6, s5, 3		#     s6 = j * 8
	add s6, s6, s0		#     s6 = tab + j * 8
	ld s7, 0(s6)		#     s7 = tab[j]
	ld s8, -8(s6)		#     s8 = tab[j-1]
	...
	blez a0, contj		# Dans le bon sens => on ne permute pas
	# Autrement, on permute:
	sd s7, -8(s6)
	sd s8, 0(s6)
...
strcmp:
	li a2, 0		# resultat
strcmp_loop:
	lbu t0,0(a0)		# char de a0
	lbu t1,0(a1)		# char de a1
