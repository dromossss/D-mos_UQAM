...
t:	.word 8, 1, 2, 10, 5, 5, -2, 2, 5, 4
	...
	.text
	la a0, t      # Charge l'adresse du tableau t
	li a1, tlen   # Nombre d'éléments (10)
	li a2, esize  # Taille d'un élément (4 octets)
	la a3, somme  # Fonction pour additionner les éléments
	li a4, 0      # Valeur de départ (0)
	call fold     # Appelle la fonction fold
	call printInt # Affiche le résultat
	li a0, '\n'
	call printChar
		#Il va ajouter chaque élément du tableau, un par un, pour obtenir la somme totale.
		#Ensuite, il affiche le résultat (40).
	# Appel de fold pour chercher l'élément minumal
	la a0, t
	li a1, tlen
	li a2, esize
	la a3, min		 # Fonction pour comparer et garder le plus petit
	lw a4, 0(a0)	 # Prend le premier élément comme point de départ
	call fold
	call printInt
	li a0, '\n'
	call printChar
	#Il compare chaque élément du tableau avec le plus petit trouvé jusqu'à maintenant.
